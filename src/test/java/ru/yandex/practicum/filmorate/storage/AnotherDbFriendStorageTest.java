...
//@ActiveProfiles("test")
public class AnotherDbFriendStorageTest {
    ...
    @Autowired
    public AnotherDbFriendStorageTest(@Qualifier("dbFriendStorage") FriendStorage friendStorage, @Qualifier("dbUserStorage") UserStorage userStorage) {
    ...
    @Test
    void testAddUserAsFriend() {
        // Пользователь 1 добавляет пользователя 2 в друзья
        friendStorage.addUserAsFriend(user1.getId(), user2.getId());

        // Проверяем, что пользователь 2 добавлен в друзья пользова
        // теля 1
        User updatedUser2 = userStorage.get(user2.getId());
        assertThat(updatedUser2.getFriends()).contains(user1.getId());//ДОЛЖНО БЫТЬ ТАК
        ...
        System.out.println("u1 friends " + updatedUser1.getFriends());
        System.out.println("u2 friends " + updatedUser2.getFriends());
    ...
    void testGetUserFriends() {
        // Пользователь 1 добавляет пользователя 2 и пользователя 3 в друзья
        friendStorage.addUserAsFriend(user1.getId(), user2.getId());
    ...
    void testGetMutualFriendsWithOtherUser() {
        ...
        // Пользователь 2 добавляет пользователя 3 в друзья
